...
// Modified to work with a web browser
// Added simple SYS_mmap2 support
...
var stdio = (function() {
  ...
  return {
    ...
    __flush_stdout: function() {
      ...
      console.log(stdout_buf); stdout_buf = '';
    },
...
let memoryCurrentPosition = null;
function syscall_mmap2(_, requested) {
  if (memoryCurrentPosition === null) {
    memoryCurrentPosition = heap_size_bytes;
  }
  const cur = memoryCurrentPosition;
  if (cur + requested > heap_size_bytes) {
    const need = Math.ceil((cur + requested - heap_size_bytes) / PAGE_SIZE);
    memory.grow(need);
    setHeap(memory);
  }
  memoryCurrentPosition += requested;
  return cur;
}
